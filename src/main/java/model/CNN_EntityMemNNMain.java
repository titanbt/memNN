...
import nn.CNN_SentenceEntityMemNN;
...
public class CNN_EntityMemNNMain {
	...
	public CNN_EntityMemNNMain(String modelFile) throws Exception
	{
	...
	public CNN_EntityMemNNMain() throws Exception
	{
		...
		xseedLLT1 = new LookupLinearTanh(windowSizeWordLookup1, wordVocab.size(), embeddingLength, embeddingLength);
		xseedLLT1.lookup.setEmbeddings(table);

		xseedLLT2 = new LookupLinearTanh(windowSizeWordLookup2, wordVocab.size(), embeddingLength, embeddingLength);
		xseedLLT2.lookup.setEmbeddings(table);

		xseedLLT3 = new LookupLinearTanh(windowSizeWordLookup3, wordVocab.size(), embeddingLength, embeddingLength);
...
//				System.err.println("targetIds.length == 0 || forwWordIds.length == 0");
			...
			CNN_SentenceEntityMemNN memNN = new CNN_SentenceEntityMemNN(
					wordIds,
					seedLookup,
					attentionCellSeed,
					entityTranformSeed,
			...
			int maxWindowSize = Math.max(xseedLLT1.lookup.inputLength,
					Math.max(xseedLLT2.lookup.inputLength, xseedLLT3.lookup.inputLength) );
			if(wordIds.length < maxWindowSize)
			{
				continue;
			}
...
//				System.err.println("targetIds.length == 0 || forwWordIds.length == 0");
	...
	public void dumpAttentionWeights(CNN_SentenceEntityMemNN memNN,
	...
	public static void main(String[] args) throws Exception
	{
		...
		CNN_EntityMemNNMain main = new CNN_EntityMemNNMain();
