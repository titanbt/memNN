...
import nn.libs.combinedLayer.SentenceConvMultiFilter;
import nn.libs.combinedLayer.LookupLinearTanh;

import java.util.Arrays;
import java.util.Random;

public class CNN_SentenceEntityMemNN implements NNInterface{
	...
	SentenceConvMultiFilter sentenceConv;

	int numberOfHops;

	public CNN_SentenceEntityMemNN()
	{
		...
		) throws Exception 
	{
		...
		sentenceConv = new SentenceConvMultiFilter(wordIds, seedLLT1, seedLLT2, seedLLT3);
		sentenceConv.link(lookupConnect);
	...
	public void backward() {
		...
		sentenceConv.backward();
	}

	public void update(double learningRate) {
		sentenceConv.update(learningRate);
		for(int i = 0; i < numberOfHops; i++)
		{
