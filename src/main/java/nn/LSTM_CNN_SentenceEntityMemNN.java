...
public class LSTM_CNN_SentenceEntityMemNN implements NNInterface{
	...
	public ConvolutionLayer[] convolutionLayers;
	...
	public LSTM_CNN_SentenceEntityMemNN()
	{
		...
		) throws Exception 
	{
		...
		convolutionLayers = new ConvolutionLayer[numberOfHops];
		for(int i = 0; i < numberOfHops; i++)
		{
			convolutionLayers[i] = new ConvolutionLayer(seedLLT1, seedLLT2, seedLLT3, wordIds.length, xTargetVec.length);
		}
	...
	public void forward() {
		...
		for(int i = 0; i < numberOfHops; i++)
		{
			if(i == 0)
			{
				convolutionLayers[i].forward(targetVec, lookupConnect.output);
				System.arraycopy(targetVec, 0, 
						(double[]) entityTransforms[i].getInput(0), 0, targetVec.length);
			}
			else
			{
				convolutionLayers[i].forward((double[]) outputs[i-1], lookupConnect.output);
				System.arraycopy((double[]) outputs[i-1], 0,
			...
			for(int j = 0; j < outputs[i].length; j++)
			{
				outputs[i][j] = ((double[]) entityTransforms[i].getOutput(0))[j] 
						+ ((double[]) convolutionLayers[i].getOutput(0))[j];
			}
		}
	}
	...
	public void backward() {
		...
		for(int i = numberOfHops - 1; i >=0 ; i--)
		{
			for(int j = 0; j < outputs[i].length; j++)
			{
				convolutionLayers[i].outputG[j] += outputGs[i][j];
				((double[]) entityTransforms[i].getOutputG(0) )[j] += outputGs[i][j];
			}
			
			entityTransforms[i].backward();
			convolutionLayers[i].backward();

			for(int j = 0; j < convolutionLayers[i].contextGs.length; j++)
			{
				lookupConnect.outputG[j] += convolutionLayers[i].contextGs[j];
			}
			...
			if(i > 0)
			{
				...
				{
					outputGs[i-1][j] += convolutionLayers[i].inputG[j]
	...
	public void update(double learningRate) {
		...
		for(int i = 0; i < numberOfHops; i++)
		{
			convolutionLayers[i].update(learningRate);
	...
	public void updateAdaGrad(double learningRate, int batchsize) {
		for(int i = 0; i < numberOfHops; i++)
		{
			convolutionLayers[i].updateAdaGrad(learningRate,batchsize);
