...
public class AddLayer implements NNInterface{
    ...
    public AddLayer(int xInputLength, int xOutputLength)
    {
    	this(xInputLength, xOutputLength, 0);
    }
    
    public AddLayer(int xInputLength, int xOutputLength, int xLinkId)
    {
	...
	public void forward() {
		...
		for(int j = 0; j < inputLength / outputLength; j++)
		{
			for(int i = 0; i < outputLength; i++)
			{
	...
	public void backward()
	{
		...
		for(int j = 0; j < inputLength / outputLength; j++)
		{
			for(int i = 0; i < outputLength; i++)
			{
				inputG[i + j * outputLength] = outputG[i];
			}
		}
	}
	...
	public Object cloneWithTiedParams() {
		// TODO Auto-generated method stub
		AddLayer clone = new AddLayer(inputLength, outputLength, linkId);
		return clone;
	}
